/*
drrt.cpp
Implementation of Drrt class. Dynamic tree is used to get random viewpoints in local area. New rrt
...
drrt_planner_ns::Drrt::Drrt(volumetric_mapping::OctomapManager* manager, DualStateGraph* graph,
...
drrt_planner_ns::Drrt::~Drrt()
{
  delete rootNode_;
  kd_free(kdTree_);
}

void drrt_planner_ns::Drrt::setParams(Params params)
{
  params_ = params;
}

void drrt_planner_ns::Drrt::setRootWithOdom(const nav_msgs::Odometry& pose)
{
...
void drrt_planner_ns::Drrt::setTerrainCLoud()
{
...
void drrt_planner_ns::Drrt::setTerrainVoxelElev()
{
  terrain_voxle_elev_.clear();
  terrain_voxle_elev_ = dual_state_frontier_->getTerrainVoxelElev();
}

int drrt_planner_ns::Drrt::getNodeCounter()
{
  return nodeCounter_;
}

bool drrt_planner_ns::Drrt::gainFound()
{
  return bestGain_ > params_.kZeroGain;
}

double drrt_planner_ns::Drrt::angleDiff(StateVec direction1, StateVec direction2)
{
  double degree;
...
double drrt_planner_ns::Drrt::getZvalue(double x_position, double y_position)
{
  int indX =
...
bool drrt_planner_ns::Drrt::inSensorRange(StateVec& node)
{
...
bool drrt_planner_ns::Drrt::inPlanningBoundary(StateVec node)
{
...
bool drrt_planner_ns::Drrt::inGlobalBoundary(StateVec node)
{
...
bool drrt_planner_ns::Drrt::generateRrtNodeToLocalFrontier(StateVec& newNode)
{
...
void drrt_planner_ns::Drrt::getNextNodeToClosestGlobalFrontier()
{
  StateVec p1, p2;
  pcl::PointXYZ p3;
                                                                                  ...
                                                                                  // of the robot
  {
    ...
    for (int j = 0; j < dual_state_frontier_->global_frontier_->size(); j++)
    {
      ...
      if (length1 > (this->manager_->getSensorMaxRange() + params_.kGainRange) ||
          fabs(p1.z() - p2.z()) > params_.kMaxExtensionAlongZ)  // Not only consider the sensor range, also take the
                                                                // node's view range into consideration
      {
        continue;  // When the node is too far away from the frontier or the difference between the z
      }            // of this node and frontier is too large, then skip to next frontier. No need to use FOV here.
      if (volumetric_mapping::OctomapManager::CellStatus::kOccupied == manager_->getVisibility(p1, p2, false))
      {
        continue;  // Only when there is no occupied voxels between the node and frontier, we consider
...
void drrt_planner_ns::Drrt::getThreeLocalFrontierPoint()  // Three local frontiers that are most close to the last
...
{                                                         // will be selected.
  ...
  for (int i = 0; i < localFrontierSize; i++)
  {
    frontierDirection[0] = dual_state_frontier_->local_frontier_->points[i].x -
                           root_[0];  // For ground robot, we only consider the direction along x-y plane
...
bool drrt_planner_ns::Drrt::remainingLocalFrontier()
{
...
bool drrt_planner_ns::Drrt::collisionCheckByTerrain(StateVec origin_point, StateVec goal_point)
{  // Collision check here cannot use pcl::kdtree
  int count = 0;
  ...
  for (int j = 0; j < check_point_num; j++)
  {
    ...
    p1.x = origin_point.x() + j * params_.kTerrainCheckDist / distance * (goal_point.x() - origin_point.x());
    p1.y = origin_point.y() + j * params_.kTerrainCheckDist / distance * (goal_point.y() - origin_point.y());
...
void drrt_planner_ns::Drrt::plannerIterate()
{
  ...
  drrt_planner_ns::Node* newParent = (drrt_planner_ns::Node*)kd_res_item_data(nearest);
  kd_res_free(nearest);
  ...
  drrt_planner_ns::Node* nearestNode = (drrt_planner_ns::Node*)kd_res_item_data(nearest_node);
  kd_res_free(nearest_node);
  ...
  else
  {
    if (volumetric_mapping::OctomapManager::CellStatus::kFree ==
        ...
        (!collisionCheckByTerrain(origin, newState)))
    {  // connection is free
      // Create new node and insert into tree
      drrt_planner_ns::Node* newNode = new drrt_planner_ns::Node;
...
void drrt_planner_ns::Drrt::plannerInit()
{
...
void drrt_planner_ns::Drrt::pruneTree(StateVec root)
{
  dual_state_graph_->pruned_graph_.vertices.clear();
  geometry_msgs::Pose p1;
...
double drrt_planner_ns::Drrt::gain(StateVec state)
{
...
void drrt_planner_ns::Drrt::clear()
{
  delete rootNode_;
  rootNode_ = NULL;
...
void drrt_planner_ns::Drrt::publishNode()
{
