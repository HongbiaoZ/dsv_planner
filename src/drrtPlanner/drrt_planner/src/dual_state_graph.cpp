...
void DualStateGraph::addNewPrunedVertex(geometry_msgs::Pose& vertex_msg, graph_utils::TopologicalGraph& graph)
{
  ...
  // Check again if there is collision between the new node and its closest node. Although this has been done for local
...
void DualStateGraph::addEdge(int start_vertex_idx, int end_vertex_idx, graph_utils::TopologicalGraph& graph)
{
  ...
  else
  {
    ...
    if ((!path_exists || (path_exists && ((dist_path / dist_edge) >= kExistingPathRatioThreshold))) &&
        ...
        (!collisionCheckByTerrain(graph.vertices[start_vertex_idx].location, graph.vertices[end_vertex_idx].location)))
    {
      Eigen::Vector3d origin;
      Eigen::Vector3d end;
...
void DualStateGraph::addGlobalEdge(int start_vertex_idx, int end_vertex_idx)
{
  ...
  else
  {
    ...
    else
    {
      ...
      // ROS_WARN("path exists of edge of length %f, where path exists of length %f", dist_edge, dist_path);
    }
  ...
  if (global_graph_.vertices.size() > 0)
  {
    for (int i = 0; i < global_graph_.vertices.size(); i++)
    {
      ...
      if (volumetric_mapping::OctomapManager::CellStatus::kFree != manager_->getCellStatusPoint(vertex_position))
      {
        ...
        //        global_graph_.vertices.erase(global_graph_.vertices.begin() + i);
        //        i--;
      }
    }
  }
    ...
    if (graph_vertex.vertex_id > 0)
    {
      ...
      if (path_information_gain > 0)
      {
        ...
        else
        {
          ...
          path_information_gain = path_information_gain / log(DTWValue_ * kDirectionCoeff);
        }
      }
...
geometry_msgs::Point DualStateGraph::getBestLocalVertexPosition()
{
  ...
  geometry_msgs::Point best_vertex_location = local_graph_.vertices[goal_id].location;*/
...
void DualStateGraph::keyposeCallback(const nav_msgs::Odometry::ConstPtr& msg)
{
  ...
  addNewGlobalVertexWithoutDuplicates(keypose);
}

void DualStateGraph::terrainCallback(const sensor_msgs::PointCloud2::ConstPtr& msg)
{
  ...
  if (graph_path->poses.size() > 2)
  {
    for (int i = 1; i < graph_path->poses.size() - 1; i++)
    {
      ...
      if (volumetric_mapping::OctomapManager::CellStatus::kFree != manager_->getLineStatus(origin, end) ||
          (kCropPathWithTerrain && collisionCheckByTerrain(origin_position, end_position)))
      {
...
bool DualStateGraph::collisionCheckByTerrain(geometry_msgs::Point origin_point, geometry_msgs::Point goal_point)
{
  int count = 0;
  ...
  for (int j = 0; j < check_point_num; j++)
  {
    ...
    p1.x = origin_point.x + j * kTerrainCheckDist * 1.5 / distance * (goal_point.x - origin_point.x);
    p1.y = origin_point.y + j * kTerrainCheckDist * 1.5 / distance * (goal_point.y - origin_point.y);
...
bool DualStateGraph::initialize()
{
  ...
  terrain_point_cloud_sub_ =
      nh_.subscribe<sensor_msgs::PointCloud2>(sub_terrain_map_topic_, 5, &DualStateGraph::terrainCallback, this);
