...
void initilization() {
  ...
  while (wp_ongoing) { // Keep publishing initial waypoint until the robot
    ...
    double dist_to_home =
        sqrt((home_point.x - current_odom_x) * (home_point.x - current_odom_x) +
...
int main(int argc, char **argv) {
  ...
  gp_command_pub =
      nh.advertise<graph_planner::GraphPlannerCommand>(gp_command_topic, 1);
  effective_plan_time_pub =
      nh.advertise<std_msgs::Float32>(effective_plan_time_topic, 1);
  total_plan_time_pub =
      nh.advertise<std_msgs::Float32>(total_plan_time_topic, 1);
  gp_status_sub = nh.subscribe<graph_planner::GraphPlannerStatus>(
      gp_status_topic, 1, gp_status_callback);
  waypoint_sub = nh.subscribe<geometry_msgs::PointStamped>(waypoint_topic, 1,
                                                           waypoint_callback);
  odom_sub = nh.subscribe<nav_msgs::Odometry>(odom_topic, 1, odom_callback);
  begin_signal_sub = nh.subscribe<std_msgs::Bool>(begin_signal_topic, 1,
                                                  begin_signal_callback);
  ...
  nhPrivate.getParam("/interface/effectivePlanTimeTopic",
                     effective_plan_time_topic);
