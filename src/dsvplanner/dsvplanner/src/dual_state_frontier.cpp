...
bool DualStateFrontier::readParameters() {
  ...
  nh_private_.getParam("/planner/terrainCloudSubTopic",
                       sub_terrain_point_cloud_topic_);
  nh_private_.getParam("/frontier/sub_graph_points_topic",
                       sub_graph_points_topic_);
  nh_private_.getParam("/frontier/pub_unknown_points_topic",
                       pub_unknown_points_topic_);
  nh_private_.getParam("/frontier/pub_global_frontier_points_topic",
                       pub_global_frontier_points_topic_);
  nh_private_.getParam("/frontier/pub_local_frontier_points_topic",
                       pub_local_frontier_points_topic_);
...
void DualStateFrontier::getUnknowPointcloudInBoundingBox(
  ...
  const StateVec center_corrected(
      kFrontierResolution * std::floor(center.x() / kFrontierResolution) +
          kFrontierResolution / 2.0,
      kFrontierResolution * std::floor(center.y() / kFrontierResolution) +
          kFrontierResolution / 2.0,
      center.z());
  ...
  for (double x_position = bbx_min.x(); x_position <= bbx_max.x();
       x_position += kFrontierResolution) {
    for (double y_position = bbx_min.y(); y_position <= bbx_max.y();
         y_position += kFrontierResolution) {
...
bool DualStateFrontier::isCleanedFrontier(pcl::PointXYZ point) {
  if (cleanedFrontier_->points.size() > 0) {
    for (int i = 0; i < cleanedFrontier_->points.size(); i++) {
      double dist = sqrt((point.x - cleanedFrontier_->points[i].x) *
                             (point.x - cleanedFrontier_->points[i].x) +
                         (point.y - cleanedFrontier_->points[i].y) *
                             (point.y - cleanedFrontier_->points[i].y) +
                         (point.z - cleanedFrontier_->points[i].z) *
                             (point.x - cleanedFrontier_->points[i].z));
      if (dist < 3) {
        return true;
      }
    }
...
bool DualStateFrontier::inSensorRangeofGraphPoints(StateVec point) {
  ...
  if (graphPoints_->points.size() > 0) {
    ...
    kdtree_->radiusSearch(check_point, kSearchRadius, pointSearchInd,
                          pointSearchDist);
    for (int i = 0; i < pointSearchInd.size(); i++) {
      StateVec node_point(graphPoints_->points[pointSearchInd[i]].x,
                          graphPoints_->points[pointSearchInd[i]].y,
...
void DualStateFrontier::gloabalFrontierUpdate() {
  ...
  point_ds_.setLeafSize(kFrontierFilterSize, kFrontierFilterSize,
                        kFrontierFilterSize);
...
void DualStateFrontier::terrainCloudAndOdomCallback(
  ...
  terrain_voxel_max_elev_.resize(kTerrainVoxelWidth * kTerrainVoxelWidth,
                                 -1000);
...
void DualStateFrontier::updateTerrainMinElevation() {
  ...
  for (int i = 0; i < terrainCloudSize; i++) {
    ...
    int indX = int((point.x - robot_position_[0] + kTerrainVoxelSize / 2) /
                   kTerrainVoxelSize) +
               kTerrainVoxelHalfWidth;
    int indY = int((point.y - robot_position_[1] + kTerrainVoxelSize / 2) /
                   kTerrainVoxelSize) +
               kTerrainVoxelHalfWidth;
    ...
    for (int dX = -1; dX <= 1; dX = dX + 2) {
      for (int dY = -1; dY <= 1; dY = dY + 2) {
        if (indX + dX >= 0 && indX + dX < kTerrainVoxelWidth &&
            indY + dY >= 0 && indY + dY < kTerrainVoxelWidth) {
          terrain_voxel_points_num_[kTerrainVoxelWidth * (indX + dX) + indY +
                                    dY]++;
          if (point.z <
              terrain_voxel_min_elev_[kTerrainVoxelWidth * (indX + dX) + indY +
                                      dY])
            terrain_voxel_min_elev_[kTerrainVoxelWidth * (indX + dX) + indY +
                                    dY] = point.z;
        }
...
void DualStateFrontier::updateTerrainElevationForKnown() {
  ...
  for (int i = 0; i < kTerrainVoxelWidth * kTerrainVoxelWidth; i++) {
    if (terrain_voxel_points_num_[i] > 0) {
      ...
      point.x = (indX - kTerrainVoxelHalfWidth) * kTerrainVoxelSize -
                kTerrainVoxelSize / 2 + robot_position_[0];
      point.y = (indY - kTerrainVoxelHalfWidth) * kTerrainVoxelSize -
    ...
    if (terrain_voxel_points_num_[i] <= 0) {
      ...
      if (terrain_elev_cloud_->points.size() > 0) {
        if (kdtree.nearestKSearch(point, 1, pointIdxNKNSearch,
          ...
          point.intensity =
              terrain_elev_cloud_->points[pointIdxNKNSearch[0]].intensity;
...
void DualStateFrontier::graphPointsCallback(
    const sensor_msgs::PointCloud2 &graph_msg) {
  graphPoints_->clear();
  pcl::fromROSMsg(graph_msg, *graphPoints_);
}

double DualStateFrontier::getZvalue(double x_position, double y_position) {
  int indX = int((x_position + kTerrainVoxelSize / 2) / kTerrainVoxelSize) +
             kTerrainVoxelHalfWidth;
  int indY = int((y_position + kTerrainVoxelSize / 2) / kTerrainVoxelSize) +
             kTerrainVoxelHalfWidth;
...
bool DualStateFrontier::initialize() {
  ...
  graph_points_sub_ =
      nh_.subscribe(sub_graph_points_topic_, 1,
                    &DualStateFrontier::graphPointsCallback, this);
  ...
  global_frontier_points_pub_ = nh_.advertise<sensor_msgs::PointCloud2>(
      pub_global_frontier_points_topic_, 1);
  local_frontier_points_pub_ = nh_.advertise<sensor_msgs::PointCloud2>(
      pub_local_frontier_points_topic_, 1);
  terrain_elev_cloud_pub_ =
      nh_.advertise<sensor_msgs::PointCloud2>("/elevation_map", 1);

  if (kExecuteFrequency_ > 0.0) {
    executeTimer_ =
        nh_private_.createTimer(ros::Duration(1.0 / kExecuteFrequency_),
                                &DualStateFrontier::execute, this);
  }
  for (int i = 0; i < kTerrainVoxelWidth * kTerrainVoxelWidth; i++) {
